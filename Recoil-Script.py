...
engine.say("TWO TO THE ONE FROM THE ONE TO THE THREE")
engine.runAndWait()
    ...
    while win32api.GetKeyState(0x01) < 0: #and win32api.GetKeyState(0x02) < 0:
        ...
        if active_weapon == 1:
            while current_bullet < len(Recoil_Ak47) and win32api.GetKeyState(0x01) < 0:
                recoil_value_x = ((Recoil_Ak47[current_bullet][0] / 4) / userSens)
                recoil_value_y = ((Recoil_Ak47[current_bullet][1] / 4) / userSens)
                mouse.move(recoil_value_x, recoil_value_y)
